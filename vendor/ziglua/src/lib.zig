                ...
                switch (fli) {
                    .@"fn" => |f| {
                        const args_ok = comptime args_ok: {
                            const f_params = f.param_types;

                            if (f_params.len != toLuaArgs.len) break :args_ok false;

                            for (0..toLuaArgs.len) |i| {
                                if (f_params[i] != @TypeOf(toLuaArgs[i])) break :args_ok false;
                            }
                ...
                switch (fli) {
                    .@"fn" => |f| {
                        const args_ok = comptime args_ok: {
                            ...
                            for (0..fromLuaArgs.len) |i| {
                                if (f_params[i] != @TypeOf(fromLuaArgs[i])) break :args_ok false;
                            }
    ...
    fn GenerateInterface(comptime function: anytype) type {
        ...
        return struct {
            pub fn interface(lua: *Lua) i32 {
                ...
                inline for (info.@"fn".param_types, 0..) |param, i| {
                    ...
                    if (comptime param_info == .pointer and param_info.pointer.size != .one) {
                        const parsed = lua.toAnyAlloc(param.?, (i + 1)) catch |err| {
                        ...
                        const parsed = lua.toAny(param.?, (i + 1)) catch |err| {
                            lua.raiseErrorStr(@errorName(err), .{});
                        };
